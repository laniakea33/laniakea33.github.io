...
★ Job 내부에서 생성된 예외는 Job을 생성한 곳 까지 전파된다. 완료여부와 상관없이 발생한다.
...
Flow는 기본적으로 코루틴 스코프 내에서 실행되며, 플로우 스코프 내에서 함부로 Dispatcher를 사용해 스레드를 전환할 수 없다. RxJava의 subscribeOn과 같이 flowOn(Dispatcher)을 통해, 해당 플로우의 실행 스레드를 미리 지정해야 한다.
